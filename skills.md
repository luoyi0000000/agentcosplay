...
版本以 pyproject.toml 为准。用户只需提供完整仓库 URL 并说“安装这个项目”。本页是可读安装指引，不是 SKILL.md，也不是另造的宿主插件协议。
...
6. 按宿主机制重新加载 MCP/Skills（AstrBot 配置仅在进程停止且已获管理权限时离线编辑）。实际发现 `character_write`、`session_control`、`runtime_context`、`turn_commit`、`memory_recall`；创建合成角色、打开专用测试 session、提交一个合成记忆、重新连接后召回；合成验收只在隔离临时库运行（仓库 scripts/check_installation.py），不向用户库写测试角色。先查看真实工具 Schema，不能猜参数。
...
仅 Marketplace 也可当前会话聊天，但不能作为完整安装的验收结果；完整安装继续使用 install.py。
