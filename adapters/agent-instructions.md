...
Use the Character Runtime MCP backend as the only character state store. Apply the workflow in `skills/character-runtime/SKILL.md` through your host's instruction/skill mechanism. The host provides language understanding and generation; the runtime provides typed state, persistence and deterministic boundaries.
