...
当前数据库 schema 仍为 1，默认导出包仍为 V1；仅显式包含私人 Companion 的包为 V2。本次不改写旧数据库；V1 包非破坏性转换、原包备份与 V2 回退限制见 docs/reference.md。数据库版本更新时必须增加显式迁移、备份和回退验证，不能靠删除数据库“升级”。未知的新数据库版本会拒绝打开，避免旧程序损坏它。
...
- Windows 文件被占用：先停止宿主再更新/卸载。跨平台路径、锁定文件和恢复已有测试，并配置三系统 CI；真实 Windows/Linux 尚未在本轮执行。
