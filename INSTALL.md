...
`python3 install.py doctor` 应输出 `ok:true`，并显示版本、数据路径和工具数量。它验证真实数据库可打开、宿主配置/Skill 未缺失或改写，并在临时数据库执行真实 stdio 工具发现和角色创建；不会向你的角色库写测试人物。

随后重新加载宿主，在宿主里发现 agentcosplay 工具和 Skill。让 Agent 使用真实工具做一轮：创建合成测试人物 → 开专用 session → 读取 context → commit 合成记忆 → 重新连接并召回 → 删除测试人物。再按你指定的角色开始聊天。**doctor 通过不等于宿主已经重新加载**；对话、QQ 路由和人格启用还需实际宿主验收。
...
当前数据库 schema 为 1，导出包 schema 为 1；本次安装更新没有数据迁移。数据库版本更新时必须增加显式迁移、备份和回退验证，不能靠删除数据库“升级”。未知的新数据库版本会拒绝打开，避免旧程序损坏它。
...
- Windows 文件被占用：先停止宿主再更新/卸载。Windows 分支有路径/配置测试，但尚未在真实 Windows 主机进行端到端验收。
